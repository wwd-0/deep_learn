...
import math
import random
caffe_root = '/home/wwd/miniconda3/envs/skl/lib/python3.7/site-packages/caffe/imagenet/'
test_root = '/home/wwd/deep_learn/face_dec/'
...
def face_detection(imgFile):
    net_full_conv = caffe.Net(os.path.join(test_root, './', 'deploy_full_conv.prototxt'),
                              os.path.join(test_root, './', 'alexnet_iter_50000_full_conv.caffemodel'),
                              caffe.TEST)
    ...
    for scale in scales:
        ...
        cv2.imwrite(test_root+'scale_img.jpg',scale_img)

        im = cv2.imread(test_root+'scale_img.jpg')
        net_full_conv.blobs['data'].reshape(1,3,scale_img.shape[1],scale_img.shape[0])
        transformer = caffe.io.Transformer({'data': net_full_conv.blobs['data'].data.shape})
        transformer.set_mean('data', np.load(caffe_root + './ilsvrc_2012_mean.npy').mean(1).mean(1))
...
if __name__ == "__main__":
    imgFile = '/home/wwd/deep_learn/face_dec/test.jpg'
